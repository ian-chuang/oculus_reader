...
class Auboi5Robot:
    ...
    def move_to_target_in_quaternion(self, pos, ori):
        """
    ...
    def get_joint_angles(self, pos, ori):
        self.check_event()
        if self.rshd >= 0 and self.connected:
            joint_radian = libpyauboi5.get_current_waypoint(self.rshd)
            return libpyauboi5.inverse_kin(self.rshd, joint_radian['joint'], pos, ori)
        else:
            logger.warn("RSHD uninitialized or not login!")
            return RobotErrorType.RobotError_NotLogin
...
def test_process_cartesian():
    logger_init()
    logger.info("{0} test beginning...".format(Auboi5Robot.get_local_time()))
    Auboi5Robot.initialize()
    robot = Auboi5Robot()
    handle = robot.create_context()
    logger.info("Context created. robot.rshd={0}".format(handle))
    ...
    try:
        ...
        if result != RobotErrorType.RobotError_SUCC:
            ...
            robot.move_to_target_in_quaternion(pos, quaternion)
